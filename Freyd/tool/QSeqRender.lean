/-
  ...
  Run:  lake env lean --run Freyd/tool/QSeqRender.lean   (writes Freyd/tool/out/*.typ)
-/
...
def main : IO Unit := do
  ...
  for (nm, s) in bookSentences do
    IO.FS.writeFile ("Freyd/tool/out/" ++ nm ++ ".typ") (render (compile s))
    IO.println ("wrote Freyd/tool/out/" ++ nm ++ ".typ")
