/-
  ...
  Run:  lake env lean --run Freyd/tool/QSeq139Render.lean   (writes Freyd/tool/out/<title>.typ)
-/

import Freyd.tool.QSeq139
...
def main : IO Unit := do
  IO.FS.createDirAll "Freyd/tool/out"
  let s := monicPairQSeq
  IO.FS.writeFile "Freyd/tool/out/qseq139_monicPair.typ" (render s)
  IO.println "wrote Freyd/tool/out/qseq139_monicPair.typ"
