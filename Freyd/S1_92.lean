...
-- (`relPullback_compose_dist` relocated to `Freyd.S1_56` — pure relation×pullback
...
/-- A PARTIAL MAP CLASSIFIER (§1.921, §1.934): an object Ω₊ together with a
    ...
    See `Freyd/S1_97.lean :: nno_of_bicartesian_data` for the full root-cause analysis (§1.988/§1.989
    /§2.542 capitalization), and `Freyd/Capitalization.lean :: capData_exists` for the §1.543 wall. -/
...
--     (Freyd.RightAdjointImage; the right adjoint f## = ∀_f built from the internal-∀ machinery);
--   · `HasSubobjectUnions`   — `toposHasSubobjectUnions` (Freyd.ToposColimits);
--   · `RegularCategory`      — `topos_is_regular_real` (Freyd.InternalForallTopos).
-- The adjunction f# ⊣ f## (i.e. InverseImage f B' ≤ A' ↔ B' ≤ radjImage f A') is
-- `radjImage_adjunction` in Freyd.RightAdjointImage, sorry-free.
