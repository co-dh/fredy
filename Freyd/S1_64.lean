...
-- ✅ DONE: `overPreLogos` and `overPositivePreLogos` in `Freyd.SlicePreTopos`.
...
-- from `Freyd.SlicePreTopos`.  Recorded as a downstream corollary there.
...
/-! ## §1.621/§1.623 Disjointness of positive coproducts — RELOCATED to S1_62
  ...
  `coprod_inl_inr_disjoint_elt`) now live in `Freyd.S1_62`, next to their natural
  ...
  here via `import Freyd.S1_62`.  Only `PreToposDisjoint` stays below, because it
...
/- §1.662 (2)→(3) `one_one_choice_to_boolean` proved in `Freyd.Diaconescu` (which imports the slice
   pre-topos instances of `Freyd.SlicePreTopos`, themselves importing this file). -/
