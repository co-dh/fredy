/-
  ...
  The strict template (`Freyd/Capitalization.lean:283`) is a ONE-LINER body:
  ...
  WRAPPERS around the committed germ-preservation facts:
    ...
    * `objInclL_preserves_products`   (`Freyd/LaxGermProducts.lean`)   → `PreservesBinaryProducts`;
    * `objInclL_preserves_equalizers` (`Freyd/LaxGermEqualizers.lean`) → `PreservesEqualizers`.
