/-
  ...
  This file works in a bare `RegularCategory 𝒞` (weaker than abelian) and delivers:
    ...
    * the Galois adjunction  `f ⊣ f*`  :  `(f_* S).le T ↔ S.le (f^* T)`   (canonical
      `existsAlong_adj`, S1_60; reused here — no local re-proof),
...
/-- The **unit** `S ≤ f* (f_* S)` of the adjunction.  The Galois adjunction `f ⊣ f*`
    `(f_* S).le T ↔ S.le (f^* T)` itself is the canonical `existsAlong_adj` (S1_60,
...
theorem le_inverseImage_directImage [HasImages 𝒞] [HasPullbacks 𝒞] {A B : 𝒞}
  ...
  (existsAlong_adj f S (DirectImage f S)).mp (Subobject.le_refl _)
...
theorem frobenius_le [HasImages 𝒞] [HasPullbacks 𝒞] {A B : 𝒞} (f : A ⟶ B)
  ...
  Subobject.le_inter
    ...
    ((existsAlong_adj f (Subobject.inter S (InverseImage f T)) T).mpr
