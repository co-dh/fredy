/-
  ...
  `Fredy/L387.lean` writes `firstUniqFn s := scanFrom s s 0`, a single left-to-right scan that calls
  ...
  `O(n²)`.  Here the SAME answer is produced by TWO passes, each an ordinary cons-fold:
    ...
    * **Pass 1 — count map.**  Carrier `C := AHashMap Nat` (char code ↦ occurrence count).  Base
      ...
      `Fredy/A6_HashMap.lean` documents — Lean proves behaviour, not asymptotics).
  ...
  Both folds are ordinary `ConsList Unit Int` recursions (`consFold_unique`, `Fredy/A6_GenFold.lean`);
