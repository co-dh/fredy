/-
  ...
  Same recipe as `Fredy/L53.lean` / `Fredy/L198.lean` (see `Fredy/leetcode.md`, skill S0/S1/S4):

  1. **Data** — the array is the initial algebra `SnocList ℤ ℤ` of `F X = ℤ + X × ℤ`
     (`Fredy.A6_SnocList`); `wrap x` is a single-element array, `snoc xs p` appends an element.
