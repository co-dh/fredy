/-
  ...
  Same recipe family as `Fredy/L56.lean` (Merge Intervals) and `Fredy/L252.lean` (Meeting Rooms),
  ...
  Mathlib-free (Lean core `List.filter`/`mem_filter`/`mem_map` + `Fredy.A6_1_RelSet` only). No
