/-
  ...
  This is a DECISION over `Fredy.A6_TreeBin`'s `Tree`, two-input like `L100` (a tree plus an `Int`
  ...
  1. **Data** — `Tree Int` (`Fredy.A6_TreeBin`) plus an `Int` target, domain `⟨Tree Int × Int⟩`.
