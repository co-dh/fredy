...
/-- `pair m t` is mono whenever its FIRST leg `m` is mono (the `snd`-leg `t` is arbitrary).
    Dual of `mono_pair_of_mono` (which needs the `snd`-leg mono); the `fst`-leg version is
    what the `P = 1` slice point needs (`t = term D` is not mono in general). -/
theorem mono_pair_of_mono_fst {T A B : 𝒞} (m : T ⟶ A) (t : T ⟶ B) (hm : Mono m) :
    Mono (pair m t) := by
  intro W g h heq
  apply hm
...
theorem factorWP_imp_wp (A : 𝒞)
  ...
  have hmbarf_mono : Mono mbar.f := mono_pair_of_mono_fst m (term D) hm
