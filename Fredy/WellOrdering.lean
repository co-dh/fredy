/-
  ...
  Freyd §1.543's last open obligation `hwall_cap` reduces (Fredy/CapitalizationTransfinite.lean) to a
  *cofinal* points-acquisition: the successor tower must point EVERY well-supported object of the
  ...
  `Fredy.Colim.Directed` / `Fredy.Inflation.OrdChain` can be any type, mathlib-free.
  ...
  * `IsWellOrder.toDirected`  — a well-order is a `Fredy.Colim.Directed` (`bound = max`), ready to index
                                `OrdChain`/`CatSystem`.
