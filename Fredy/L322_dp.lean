/-
  ...
  dynamic-programming theorem `Fredy.A9_2.dynamic_programming_inf`, THROUGH THE GENERIC DP
  DRIVER `Fredy.AutoDeriveDP` (auto-derive increment 2).
  ...
  * the EXECUTABLE `dp` (from `Fredy.L322`, fuel recursion) with measure the amount itself;
  ...
  * the SPEC BRIDGE: the driver's generic reachability `Steps` is `Achievable`
  ...
  `solve_correct_inf` — the same statement as before, and as `Fredy.L322.solve_correct` —
