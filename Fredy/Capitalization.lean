...
structure CapStep (S : Type u) [Cat.{u} S] [PreRegularCategory S] where
  ...
  /-- §1.543 REGULAR upgrade: the next stage `T = S*` HAS IMAGES.  The successor is the directed
      colimit of slices of the regular inflation `Infl S` (`overHasImages`/`inflRegular`), so its
      colimit `HasImages` is a `colimitHasImages` of that inner system (`chainSliceHasImages`).
      Carrying this per rung is what makes every ω-tower stage REGULAR, hence the colimit `Ā`
      `RegularCategory` (the `hi` the `capData_of_tower_regular` consumes). -/
  stepImages : @HasImages T catT
...
noncomputable def nextStepOfEnum {S : Type u} [Cat.{u} S] [hreg : RegularCategory S]
  ...
  exact
    { T := (chainSliceSystem P).Obj
      ...
      -- §1.543 REGULAR: `T = S*` has images.  `chainSliceHasImages` builds the colimit `HasImages`
      -- of the inner slice system from per-stage `overHasImages` (each `Over (chain i : Infl S)` is a
      -- slice of the regular inflation `Infl S`, `inflRegular`) + the WS-suffix faithfulness `hwsuf`;
      -- the colimit pullbacks come from `preT` (= `chainSlicePreRegularWS`).
      stepImages :=
        letI : @HasPullbacks (chainSliceSystem P).Obj (colimitCat _ (chainSliceCoherent P)) :=
          (chainSlicePreRegularWS (𝒞 := S) P hwsuf).toHasPullbacks
        chainSliceHasImages (𝒞 := S) P (fun {i j} hij => hwsuf hij) }
...
/-- **§1.543 SUCCESSOR step (regular).**  `RegularCategory S ⟹ RegularCategory (nextStep S).T`: the
    relative-capitalization successor `S ↦ S*` PRESERVES regularity.  Pre-regularity is `CapStep.preT`
    (always); the new `CapStep.stepImages` supplies the images, derived from the inner slice colimit
    (`chainSliceHasImages`: each `Over (chain i : Infl S)` is a slice of the regular inflation `Infl S`,
    `inflRegular`/`overHasImages`).  This is the recursive engine that makes every ω-tower stage
    regular, hence the colimit `Ā` `RegularCategory` (the `hi` of `capData_of_tower_regular`). -/
noncomputable def nextStepRegular (S : RegBundle.{u}) :
    @RegularCategory (nextStep S).T (nextStep S).catT :=
  letI : Cat (nextStep S).T := (nextStep S).catT
  letI : PreRegularCategory (nextStep S).T := (nextStep S).preT
  letI : HasImages (nextStep S).T := (nextStep S).stepImages
  { toHasTerminal := PreRegularCategory.toHasTerminal
    toHasBinaryProducts := PreRegularCategory.toHasBinaryProducts
    toHasPullbacks := PreRegularCategory.toHasPullbacks
    toHasImages := (nextStep S).stepImages
    toPullbacksTransferCovers := PreRegularCategory.toPullbacksTransferCovers }

/-- The successor target `(nextStep S).T = S*` HAS IMAGES (the `CapStep.stepImages` field, named). -/
noncomputable def nextStep_hasImages (S : RegBundle.{u}) :
    @HasImages (nextStep S).T (nextStep S).catT :=
  (nextStep S).stepImages
