/-
  ...
  `Fredy/L128.lean` computes the answer by SORTING (`L242.isort`, O(n²) insertion sort) and then a
  ...
  **Correctness is proved directly against `L128`'s membership spec** (the two-phase structure does not
  ...
  Mathlib-free (imports `Fredy.A6_HashMap`).  Headline axioms ⊆ {propext, Quot.sound}.
-/
