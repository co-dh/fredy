/-
  ...
  `Fredy/L242.lean` packages the answer via a hand-written `isort : List Int → List Int`
  ...
  (`Fredy/A6_GenFold.lean`) PRODUCES this fold as `cataR (consScalarAlg g st)` — insertion sort is
