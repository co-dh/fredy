/-
  ...
  1. **The executable model `FinRel`** — objects are finite cardinalities (`FinObj`, carrier
     ...
     decidable, `#eval`-runnable counterpart of `Rel(Set)` (`Fredy.A6_1_RelSet`, whose
  ...
  2. **The term language `RE`** — a deep AST with primitive constructors
     ...
     (§2.331/§2.41), `min R = ∋ ∩ (∋°\R)` and `max R = min R°` (B&dM §7.1, `Fredy.A7_1`).
...
/-- B&dM §7.1 `min R = ∋ ∩ (∋°\R)` (`Fredy.A7_1.minRel` verbatim). -/
def minRelE {a : FinObj} (R : RE a a) : RE (pow a) a :=
  .meet (.eps a) (leftDivE (.conv (.eps a)) R)

/-- B&dM §7.1 `max R = min R°` (`Fredy.A7_1.maxRel` verbatim). -/
...
/-- `eval (minRelE R)` is B&dM's `min R = ∋ ∩ (∋°\R)` (`Fredy.A7_1.minRel`'s body, which is
...
/-! ## Demo 3 — LeetCode 121 (Best Time to Buy and Sell Stock) run as its SPEC TERM
  ...
  `Fredy.L121` proves `solve = A spec ≫ maxRel D` in `Rel(Set)` — a PROOF, never executed.
...
/-! ## Demo 4 — the DERIVED PROGRAM run by STRUCTURAL FOLD (polynomial, NO powerset)
  ...
  derivation `solve = A spec ≫ max D` (proven in `Fredy.L121`); here we RUN both on the same
  small instance and check they agree — the derivation's correctness, now runnable on both sides. -/

namespace ProgEval

/-- Non-empty snoc-list — the initial algebra of `F A X = A ⊕ X × A`, the datatype `Fredy.L121`
...
/-! ### LC 121's derived program as a runnable term (its exact `algFn`, `Fredy.L121`) -/
...
/-- A fuel-bounded HYLOMORPHISM over the same functor: unfold seed `s` via coalgebra `g` (a leaf
    ...
    recursion depth, so it is total with no well-foundedness proof (cf. `Fredy.A6_GenHylo`). -/
