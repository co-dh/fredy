...
theorem stageInclL_g''_factor (A : S) {U U' : WSList S} (hbU : (wsDirected S).le W.base U)
    ...
    ∃ (iL : @Quiver.Hom _ (laxColimCat (laxOfProjSystem' (cofinalProjSystem (S := S)))
              (coherentProj (cofinalProjSystem (S := S)))).toCategoryStruct.toQuiver
            ⟨U, xE'⟩ ⟨U', (laxOfProjSystem' (cofinalProjSystem (S := S))).F hUU' xE'⟩)
      (jR : @Quiver.Hom _ (laxColimCat (laxOfProjSystem' (cofinalProjSystem (S := S)))
              (coherentProj (cofinalProjSystem (S := S)))).toCategoryStruct.toQuiver
  ...
  have hpg_eq :
    ...
    show _ = _
    simp only [CategoryTheory.Category.assoc, pushFibre]
  ...
  have hiLeft : IsIso (transApp L ((wsDirected S).refl U) hUU' xE'
    ...
    isIso_comp (transApp_isIso L ((wsDirected S).refl U) hUU' xE')
      (functor_preserves_iso
        (bundledFunctor (hF := L.functF hUU') (L.F hUU'))
        (reflApp L xE') (reflApp_isIso L xE'))
  have hiRight : IsIso (@Functor.map _ _ _ _ _ (L.functF hUU') _ _ (isoInv (reflApp_isIso L yA))
    ...
    isIso_comp
      (functor_preserves_iso
        (bundledFunctor (hF := L.functF hUU') (L.F hUU')) (isoInv (reflApp_isIso L yA))
  ...
  refine ⟨alignGerm L hL xE' ((wsDirected S).trans ((wsDirected S).refl U) hUU')
          ...
          ?_, ?_, ?_⟩
  · have h := functor_preserves_iso
      (bundledFunctor (hF := stageInclFunctorL L hL U') (fun x => (⟨U', x⟩ : Obj L)))
      (transApp L ((wsDirected S).refl U) hUU' xE'
...
/-! ## Phase 3 — the genuine §1.546 density core `RicherSliceMiss`
  ...
  square and joint-monicity of `(fst,snd)`, exhibit a section of `g''.left` (i.e. `g''.left` split-epi
  ...
  interface (`richerSliceSection`):
    (a) — SORRY-FREE.  The codomain `F hUU' (F hbU (terminalSliceObj A))` is `baseChangeObj snd
        ...
        pair (cnD.π₁ ≫ m.left ≫ fst) cnD.π₂`, `m := g'' ⊚ pushTerminalSlice_iso⁻¹`.
...
theorem bcSlice_isPullback {𝒞 : Type u} [CategoryTheory.Category.{u} 𝒞] [HasTerminal 𝒞] [HasBinaryProducts 𝒞]
  ...
  · intro v hv₁ hv₂
    have ev1 : v ≫ (fst : prod C P' ⟶ C) = (pair (d.π₁ ≫ (fst : prod C P ⟶ C)) d.π₂) ≫ fst := by
      rw [fst_pair, ← hv₁]; show _ = (v ≫ pair fst (snd ≫ q)) ≫ fst; rw [CategoryTheory.Category.assoc, fst_pair]
...
theorem baseChangeTransNatIso_app_f_π₁_cast {C D E : S} (g : C ⟶ D) (g' : E ⟶ C) (X : Over D)
    ...
    (Eq.ndrec
        ...
        (⟨(Freyd.LaxColim.baseChangeTransNatIso g g').nat.app X,
          (Freyd.LaxColim.baseChangeTransNatIso g g').isIso X⟩) hZ).fst.left
...
theorem richerSliceSection (W : WSCover S) (aT : Tok S)
  ...
  have hsp : tSelectProj Prod.snd U'.1 U.1 hUU' = (snd : prod A P ⟶ P) := by
    ...
    rw [tSelectProj_head_notin Prod.snd aT U.1 U.1 hUU' hAU (fun t hB => hB),
        tSelectProj_refl Prod.snd U.2.1 (fun t hB => hB), CategoryTheory.Category.comp_id]
  ...
  have hm_niso : ¬ @IsIso (Over P) _ _ _ m := by
    ...
    have he : g'' = @Cat.comp (Over P) _ _ _ _ m pIso := by
      ...
      have hpp : pInv.left ≫ pIso.left = 𝟙 _ :=
        congrArg CategoryTheory.CommaMorphism.left hp2
      show g''.left = (g''.left ≫ pInv.left) ≫ pIso.left
      rw [CategoryTheory.Category.assoc, hpp, CategoryTheory.Category.comp_id]
  ...
  let mf' : cnD.pt ⟶ prod A (prod A P) :=
    pair (cnD.π₁ ≫ m.left ≫ (fst : prod A P ⟶ A)) cnD.π₂
  have hmf1 : mf' ≫ (fst : prod A (prod A P) ⟶ A) =
      cnD.π₁ ≫ m.left ≫ (fst : prod A P ⟶ A) := fst_pair _ _
  ...
  have key : ∀ (s : (prod A P) ⟶ cnD.pt), s ≫ cnD.π₂ = 𝟙 (prod A P) →
  ...
  let align : @Quiver.Hom (Obj L) (laxColimCat L hL).toCategoryStruct.toQuiver
      (@HasTerminal.one _ (uniformTargetCat W) (uniformStepTarget_preRegular W).toHasTerminal)
  ...
  let z : @Quiver.Hom (Obj L) (laxColimCat L hL).toCategoryStruct.toQuiver
      ⟨U', (T.ht U').one⟩ ⟨U', L.F hUU' xE'⟩ :=
  ...
  have hz : @Cat.comp (Obj L) (laxColimCat L hL) _ _ _ z
    ...
    calc
      Cat.comp (Cat.comp alignInv y')
          (stageInclL L hL (pushFibre W A hbU hUU' g'')) =
          Cat.comp alignInv (Cat.comp y'
            (stageInclL L hL (pushFibre W A hbU hUU' g''))) :=
        CategoryTheory.Category.assoc _ _ _
      _ = Cat.comp alignInv x' := by rw [hy']
      _ = stageInclL L hL (sfp ⊚ cod) := by
        change Cat.comp alignInv (Cat.comp align (stageInclL L hL (sfp ⊚ cod))) = _
        calc
          Cat.comp alignInv (Cat.comp align (stageInclL L hL (sfp ⊚ cod))) =
              Cat.comp (Cat.comp alignInv align) (stageInclL L hL (sfp ⊚ cod)) :=
            (Cat.assoc _ _ _).symm
          _ = Cat.comp (𝟙 _) (stageInclL L hL (sfp ⊚ cod)) :=
            congrArg (fun q => Cat.comp q (stageInclL L hL (sfp ⊚ cod))) halign2
          _ = _ := Cat.id_comp _
  ...
  -- section `t.left` internally and routes through `baseChange_freshFactor_missed`).  So the Step-2
  ...
  -- conjugated arrows in `Over (∏N)`.  The `pushHom` `.left`-conjugation is NO LONGER opaque: the
  ...
  --   • `proj_pushHom_f_π₂` : `(pushHom g).left ≫ (_pb (proj (trans hjk hkm)) y).π₂
  --        = (_pb (proj (trans hik hkm)) x).π₂` — `pushHom.left` preserves the `∏N`-structure map
  --     ON THE NOSE (so each layer IS an over-`∏N` arrow with computable structure leg).
  --   • `proj_pushHom_f_π₁` : `(pushHom g).left ≫ (transApp hjk hkm y).left ≫ outerπ₁
  --        = (transApp hik hkm x).left ≫ outerπ₁ ≫ g.left` — `pushHom.left` intertwines the CONTENT
  --     projection with the underlying `g.left` (the §1.546 content arrow), explicitly via the
  --     concrete pullback `lift`s of `transApp`/`baseChangeMap` (`transApp_f_π₁π₁₀`,
  --     `baseChangeMap_f_π₁`).
  -- So `(pushHom … wa).left` over `∏N` IS now an explicit composite of `baseChangeMap` underlying
  ...
  -- and the section `t.left` from `hstage` projected through `proj_pushHom_f_π₁`/`π₂` and precomposed by
  -- `ψ⁻¹`.  `m̄` is proper at `N` via `L_cons`/`projStage_conservative_full` on `g''` (`hniso`).
  -- This step is pure pullback bookkeeping over the now-explicit `.left` legs — no opaque coherence
  ...
  have hm_N_niso : ¬ @IsIso (Over (listProd (N.1.map Prod.snd))) _ _ _ m_N := by
    ...
    have hpfN_iso : IsIso pfN := by
      ...
      have heq : (@Cat.comp (Over (listProd (N.1.map Prod.snd))) _ _ _ _ m_N Θ) = pfN := by
        change Cat.comp (Cat.comp pfN (isoInv hΘiso)) Θ = pfN
        calc
          Cat.comp (Cat.comp pfN (isoInv hΘiso)) Θ =
              Cat.comp pfN (Cat.comp (isoInv hΘiso) Θ) := Cat.assoc _ _ _
          _ = Cat.comp pfN (𝟙 _) := congrArg (Cat.comp pfN) (inv_isoInv_comp hΘiso)
          _ = pfN := Cat.comp_id _
      rwa [heq] at hmΘ
  ...
  let bcGen : OverHom (sliceEmbedObj PN A)
    ...
    CategoryTheory.Over.homMk
      (U := sliceEmbedObj PN A)
      (V := baseChangeObj (tSelectProj Prod.snd (N.1.erase aT) U.1 hUe)
        (sliceEmbedObj P A))
      ((HasPullbacks.has ((sliceEmbedObj P A).hom)
        (tSelectProj Prod.snd (N.1.erase aT) U.1 hUe)).lift bcGenCone)
      (by simpa [sliceEmbedObj] using
        (HasPullbacks.has ((sliceEmbedObj P A).hom)
          (tSelectProj Prod.snd (N.1.erase aT) U.1 hUe)).lift_snd bcGenCone)
  have bcGen_iso : @IsIso (Over PN) _ _ _ bcGen := by
    apply overIso_of_underlying
    change IsIso ((HasPullbacks.has ((sliceEmbedObj P A).hom)
      (tSelectProj Prod.snd (N.1.erase aT) U.1 hUe)).lift bcGenCone)
  ...
  have hsplit2 : tSelectProj Prod.snd N.1 U'.1 hUN' ≫ (snd : prod A P ⟶ P)
    ...
    rw [show tSelectProj Prod.snd N.1 U'.1 hUN' = _ from hsplit, CategoryTheory.Category.assoc, snd_pair]
  -- ════════════════════════════════════════════════════════════════════════════════════════════
  ...
  --       P and at PN) reconcile the `sliceEmbedObj`/`baseChangeObj` codomains; `pfN`'s `.left`-legs are
  ...
  --       ⟨cnD_N.pt, π₂⟩ (sliceEmbedObj (A×PN) A)` with `m̄_N.left = mf'_N` — the consumer's cone data.
  ...
  have hdStep2ψ_iso : @IsIso (Over (listProd (N.1.map Prod.snd))) _ _ _ dStep2ψ :=
    functor_preserves_iso (bundledFunctor (baseChangeObj ψ)) dStep2 hdStep2_iso
  ...
  have hmC_niso : ¬ @IsIso (Over PN) _ _ _ mC := by
    ...
    have he : baseChangeMap (tSelectProj Prod.snd (N.1.erase aT) U.1 hUe) m
      ...
      simp [mC, Cat.assoc, inv_isoInv_comp]
  ...
  let mf'N : cnDN.pt ⟶ prod A (prod A PN) :=
    pair (cnDN.π₁ ≫ mC.left ≫ (fst : prod A PN ⟶ A)) cnDN.π₂
  have hmf1N : mf'N ≫ (fst : prod A (prod A PN) ⟶ A) = cnDN.π₁ ≫ mC.left ≫ (fst : prod A PN ⟶ A) :=
    fst_pair _ _
  ...
  let mbarN : OverHom (CategoryTheory.Over.mk cnDN.π₂)
      (sliceEmbedObj (prod A PN) A) :=
    CategoryTheory.Over.homMk mf'N (by
      show mf'N ≫ (snd : prod A (prod A PN) ⟶ prod A PN) = cnDN.π₂
      exact hmf2N)
  -- ════════════════════════════════════════════════════════════════════════════════════════════
  ...
  -- reading both legs of `t.left` off `proj_pushHom_f_π₁`/`proj_pushHom_f_π₂` by `pb_hom_ext`.  This is
  -- the genuine multi-screen §1.546 descent; every primitive it needs is in scope Sorry-free.
  have hpt : ∃ t : OverHom (overTerm (prod A PN)) (CategoryTheory.Over.mk cnDN.π₂),
    ...
    -- (A×PN) A)` equation) holds iff its underlying `A×PN`-arrow `t.left ≫ mf'N = pair fst id`.  By the
    -- product universal property (the eta law) and `hmf1N`/`hmf2N`, that splits into:
    --   • `A`-leg : `t.left ≫ cnDN.π₁ ≫ mC.left ≫ fst = fst`  (the FRESH coordinate — the §1.546 content);
    --   • `PN`-structure leg : `t.left ≫ cnDN.π₂ = id`       (the over-`A×PN` point law, = `t.w`).
    -- So it suffices to build `q : A×PN ⟶ Dbar.dom` (a section of `Dbar.hom` over `A×PN` via `snd`)
    -- with `q ≫ Dbar.hom = snd` and `q ≫ mC.left ≫ fst = fst`; then `t.left := the pullback lift` of the
    ...
    obtain ⟨q, hqstruct, hqfresh⟩ :
        ∃ q : prod A PN ⟶ Dbar.left,
          q ≫ Dbar.hom = (snd : prod A PN ⟶ PN) ∧ q ≫ mC.left ≫ (fst : prod A PN ⟶ A) = fst := by
      -- ════════════════════════════════════════════════════════════════════════════════════════
      ...
      --   `s : A×PN ⟶ cnDN.pt`  with  `s ≫ cnDN.π₂ = id`  and  `s ≫ cnDN.π₁ ≫ mC.left ≫ fst = fst`.
      ...
      obtain ⟨s, hs₂, hsA⟩ :
          ∃ s : prod A PN ⟶ cnDN.pt,
            s ≫ cnDN.π₂ = 𝟙 (prod A PN) ∧
              s ≫ cnDN.π₁ ≫ mC.left ≫ (fst : prod A PN ⟶ A) = fst := by
      ...
      --     (CapitalizationLaxColimit.lean) give the two pullback legs of each `(pushHom _).left` over
      ...
      --   • routing `(pushHom … z₀).left` through `descent.left` and the `ψ`-iso yields `s` over `A×PN`; its
      ...
      -- documents (lines 525–635, 706–845) — needing the DESCENT EQUATION relating `m_N.left` (over ∏N,
      -- content `factorProj N A = ψ ≫ fst` via `proj_pushHom_f_π₁`) to `cnDN.π₁ ≫ mC.left` (over A×PN).
      -- The `baseChangeTransNatIso` comparison legs are now PUBLIC (Step 1, CapitalizationLaxColimit):
      --   `baseChangeTransNatIso_app_f`     : `(…nat.app X).left = _transFwdf …` (defeq witness);
      ...
      --   ⊢ ∃ s : A×PN ⟶ cnDN.pt, s ≫ cnDN.π₂ = id ∧ s ≫ cnDN.π₁ ≫ mC.left ≫ fst = fst
      -- The stage-`N` representative of the colimit factor `z`, pushed to `N`.
        ...
        have hsrcEq : L.F ((wsDirected S).trans b.2.1 hbN) (T.ht U').one
            = CategoryTheory.Over.mk srcPB.cone.π₂ := rfl
        ...
        let srcCone : Cone (𝒞 := S) (overTerm (listProd (U'.1.map Prod.snd))).hom
          ...
          ⟨prod A PN, a ≫ tSelectProj Prod.snd N.1 U'.1 ((wsDirected S).trans b.2.1 hbN), a, by
            show (a ≫ tSelectProj Prod.snd N.1 U'.1 ((wsDirected S).trans b.2.1 hbN)) ≫ 𝟙 _
        ...
        have hcodPt : (baseChangeObj snd Dbar).left = cnDN.pt := rfl
        -- `s := r ≫ zNd.left ≫ codPB.π₁`, lands in `(bc snd Dbar).dom = cnDN.pt`.
        let s : prod A PN ⟶ cnDN.pt :=
          r ≫ (zNd.left ≫ codPB.cone.π₁ : srcPB.cone.pt ⟶ cnDN.pt)
        ...
        · -- structure leg: `s ≫ cnDN.π₂ = id`.
          ...
          have hzw : zNd.left ≫ codPB.cone.π₂ = srcPB.cone.π₂ :=
            CategoryTheory.Over.w zNd
          have hrw : r ≫ srcPB.cone.π₂ = a := srcPB.lift_snd srcCone
          show (r ≫ (zNd.left ≫ codPB.cone.π₁)) ≫ cnDN.π₂ = 𝟙 (prod A PN)
          calc (r ≫ (zNd.left ≫ codPB.cone.π₁)) ≫ cnDN.π₂
              = r ≫ zNd.left ≫ codPB.cone.π₁ ≫ cnDN.π₂ := by rw [CategoryTheory.Category.assoc, CategoryTheory.Category.assoc]
            _ = r ≫ zNd.left ≫ codPB.cone.π₂ ≫ ψ := by rw [hw]
            _ = r ≫ (zNd.left ≫ codPB.cone.π₂) ≫ ψ := by rw [CategoryTheory.Category.assoc]
            _ = r ≫ srcPB.cone.π₂ ≫ ψ := by rw [hzw]
            _ = (r ≫ srcPB.cone.π₂) ≫ ψ := by rw [CategoryTheory.Category.assoc]
            _ = a ≫ ψ := by rw [hrw]
            _ = 𝟙 (prod A PN) := inv_isoInv_comp hψiso
        · -- fresh leg: `s ≫ cnDN.π₁ ≫ mC.left ≫ fst = fst`.
          show (r ≫ (zNd.left ≫ codPB.cone.π₁)) ≫ cnDN.π₁ ≫ mC.left ≫ (fst : prod A PN ⟶ A) = fst
          ...
          -- now machine-checked Sorry-free: the section `s := r ≫ zNd.left ≫ codPB.π₁` (`r` the ψ⁻¹-cone
          ...
          -- `hbridge`: the deep content projection `zNd.left ≫ codPB.π₁ ≫ cnDN.π₁ ≫ mC.left ≫ fst`, traced
          -- back through `descent = dStep1inv ⊚ dStep3' ⊚ dStep2ψ` (decomposable `.left`-wise, `hdf` below
          ...
          --   ⊢ (zNd.left ≫ codPB.cone.π₁) ≫ cnDN.π₁ ≫ mC.left ≫ fst = srcPB.cone.π₂ ≫ ψ ≫ fst
          have hbridge : (zNd.left ≫ codPB.cone.π₁) ≫ cnDN.π₁ ≫ mC.left ≫ (fst : prod A PN ⟶ A)
            ...
            -- `(isoInv bcGen_iso).left ≫ fst = bcPB_P.π₁ ≫ fst`: from `lift_fst` (bcGen.left ≫ π₁ =
            -- pair fst (snd≫q')) cancelled by `(isoInv bcGen_iso).left ≫ bcGen.left = id`.
            have hbcGenInv_fst : (isoInv bcGen_iso).left ≫ (fst : prod A PN ⟶ A)
                = bcPB_P.cone.π₁ ≫ (fst : prod A P ⟶ A) := by
              have hπ₁ : bcPB_P.cone.π₁
                  = (isoInv bcGen_iso).left ≫ pair (fst : prod A PN ⟶ A)
                      ((snd : prod A PN ⟶ PN) ≫ q') := by
                rw [show pair (fst : prod A PN ⟶ A) ((snd : prod A PN ⟶ PN) ≫ q')
                      = bcGen.left ≫ bcPB_P.cone.π₁ from (bcPB_P.lift_fst bcGenCone).symm,
                    ← CategoryTheory.Category.assoc,
                    show (isoInv bcGen_iso).left ≫ bcGen.left = 𝟙 _ from
                      congrArg CategoryTheory.CommaMorphism.left (inv_isoInv_comp bcGen_iso), CategoryTheory.Category.id_comp]
              rw [hπ₁, CategoryTheory.Category.assoc, fst_pair]
            -- the `bc q' xE'` pullback = `Dbar`'s pullback (of `xE'.hom` along `q'`).
            let bcPB_E := HasPullbacks.has (𝒞 := S) (xE'.hom) q'
            -- content law: `mC.left ≫ fst = bcPB_E.π₁ ≫ m.left ≫ fst` (the A-coordinate of `m`, base-changed).
            have hmC_fst : mC.left ≫ (fst : prod A PN ⟶ A)
                = bcPB_E.cone.π₁ ≫ m.left ≫ (fst : prod A P ⟶ A) := by
              -- `mC.left = (baseChangeMap q' m).left ≫ (isoInv bcGen_iso).left`.
              show ((baseChangeMap q' m).left ≫ (isoInv bcGen_iso).left) ≫ (fst : prod A PN ⟶ A) = _
              rw [CategoryTheory.Category.assoc, hbcGenInv_fst, ← CategoryTheory.Category.assoc]
              -- `(baseChangeMap q' m).left ≫ bcPB_P.π₁ = bcPB_E.π₁ ≫ m.left` (base-change content `lift_fst`).
              rw [show (baseChangeMap q' m).left ≫ bcPB_P.cone.π₁ = bcPB_E.cone.π₁ ≫ m.left from
                    bcPB_P.lift_fst (baseChangeCone q' m), CategoryTheory.Category.assoc]
            -- rewrite the goal's `mC.left ≫ fst` by `hmC_fst`; the residual is the DESCENT-CONTENT
            ...
            rw [show cnDN.π₁ ≫ mC.left ≫ (fst : prod A PN ⟶ A)
                  = cnDN.π₁ ≫ bcPB_E.cone.π₁ ≫ m.left ≫ (fst : prod A P ⟶ A) from by
                rw [show mC.left ≫ (fst : prod A PN ⟶ A)
                      = bcPB_E.cone.π₁ ≫ m.left ≫ (fst : prod A P ⟶ A) from hmC_fst]]
            -- REMAINING (single isolated §1.546 descent-content residual).  EXACT goal:
            --   ⊢ (zNd.left ≫ codPB.cone.π₁) ≫ cnDN.π₁ ≫ bcPB_E.cone.π₁ ≫ m.left ≫ fst
            --       = srcPB.cone.π₂ ≫ ψ ≫ fst
            ...
            -- e.g. `dStep2ψ.left ≫ codPB.π₁ = (ψ-pullback of dStep2 source).π₁ ≫ dStep2.left` by
            ...
            have e0inv : dStep1inv.left
                  ≫ (HasPullbacks.has xE'.hom (g₁ ≫ (snd : prod A P ⟶ P))).cone.π₁
                = (HasPullbacks.has (baseChangeObj snd xE').hom g₁).cone.π₁
                    ≫ (HasPullbacks.has xE'.hom (snd : prod A P ⟶ P)).cone.π₁ := by
              have hid : dStep1inv.left ≫ dStep1.left = 𝟙 _ :=
                congrArg CategoryTheory.CommaMorphism.left (inv_isoInv_comp hdStep1_iso)
              calc dStep1inv.left
                      ≫ (HasPullbacks.has xE'.hom (g₁ ≫ (snd : prod A P ⟶ P))).cone.π₁
                  = dStep1inv.left ≫ dStep1.left
                      ≫ (HasPullbacks.has (baseChangeObj snd xE').hom g₁).cone.π₁
                        ≫ (HasPullbacks.has xE'.hom (snd : prod A P ⟶ P)).cone.π₁ := by rw [e0]
                _ = (dStep1inv.left ≫ dStep1.left)
                      ≫ (HasPullbacks.has (baseChangeObj snd xE').hom g₁).cone.π₁
                        ≫ (HasPullbacks.has xE'.hom (snd : prod A P ⟶ P)).cone.π₁ :=
                    (CategoryTheory.Category.assoc _ _ _).symm
                _ = (HasPullbacks.has (baseChangeObj snd xE').hom g₁).cone.π₁
                      ≫ (HasPullbacks.has xE'.hom (snd : prod A P ⟶ P)).cone.π₁ := by
                    rw [hid, CategoryTheory.Category.id_comp]
            -- chain the legs into the master descent-content reduction.
            have hdescent_content : descent.left ≫ codPB.cone.π₁ ≫ cnDN.π₁ ≫ bcPB_E.cone.π₁
                = (HasPullbacks.has (baseChangeObj snd xE').hom g₁).cone.π₁
                    ≫ (HasPullbacks.has xE'.hom (snd : prod A P ⟶ P)).cone.π₁ := by
              show (dStep1inv.left ≫ dStep3'.left ≫ dStep2ψ.left) ≫ codPB.cone.π₁ ≫ cnDN.π₁ ≫ bcPB_E.cone.π₁
                = _
              calc (dStep1inv.left ≫ dStep3'.left ≫ dStep2ψ.left)
                      ≫ codPB.cone.π₁ ≫ cnDN.π₁ ≫ bcPB_E.cone.π₁
                  = dStep1inv.left ≫ dStep3'.left
                      ≫ (dStep2ψ.left ≫ codPB.cone.π₁) ≫ cnDN.π₁ ≫ bcPB_E.cone.π₁ := by
                    simp only [CategoryTheory.Category.assoc]
                _ = dStep1inv.left ≫ dStep3'.left
                      ≫ (HasPullbacks.has (baseChangeObj ((snd : prod A PN ⟶ PN) ≫ q') xE').hom ψ).cone.π₁
                        ≫ dStep2.left ≫ cnDN.π₁ ≫ bcPB_E.cone.π₁ := by rw [e1]; simp only [CategoryTheory.Category.assoc]
                _ = dStep1inv.left ≫ dStep3'.left
                      ≫ (HasPullbacks.has (baseChangeObj ((snd : prod A PN ⟶ PN) ≫ q') xE').hom ψ).cone.π₁
            ...
            have hzNdf : zNd.left = (hcodObj ▸ zN).left ≫ descent.left := rfl
            rw [show (zNd.left ≫ codPB.cone.π₁) ≫ cnDN.π₁ ≫ bcPB_E.cone.π₁ ≫ m.left ≫ (fst : prod A P ⟶ A)
                  = (hcodObj ▸ zN).left
                      ≫ (descent.left ≫ codPB.cone.π₁ ≫ cnDN.π₁ ≫ bcPB_E.cone.π₁) ≫ m.left ≫ fst from by
                rw [hzNdf]; simp only [CategoryTheory.Category.assoc], hdescent_content]
            ...
            --   (hcodObj ▸ zN).left ≫ (srcDeep₁ ≫ srcDeep₂) ≫ m.left ≫ fst = srcPB.π₂ ≫ ψ ≫ fst
            ...
            have gen : ∀ {Z Y Y' : L.A N} (e : Y = Y') (f : Z ⟶ Y),
                (e ▸ f) = f ≫ eqToHom e := by
              intro Z Y Y' e f; subst e; rw [eqToHom_refl, CategoryTheory.Category.comp_id]
            have hcastf : (hcodObj ▸ zN).left = zN.left ≫ (eqToHom hcodObj).left :=
              congrArg CategoryTheory.CommaMorphism.left (gen hcodObj zN)
            ...
            have hbcInner : ∀ {X Y : Over (listProd (U'.val.map Prod.snd))} (e : X = Y),
                (eqToHom (congrArg (fun Z => baseChangeObj g₁ Z) e)).left
                    ≫ (_pb g₁ Y).cone.π₁
                  = (_pb g₁ X).cone.π₁ ≫ (eqToHom e).left := by
              intro X Y e; subst e
              rw [eqToHom_refl, eqToHom_refl]
              show 𝟙 _ ≫ _ = _ ≫ 𝟙 _
              rw [CategoryTheory.Category.id_comp, CategoryTheory.Category.comp_id]
            ...
            have hrec : zN.left
                  ≫ (_pb (cofinalProjSystem.proj ((wsDirected S).trans b.2.2 hbN))
                        (L.F hUU' xE')).cone.π₁
                = (transApp (laxOfProjSystem' cofinalProjSystem) b.2.1 hbN (T.ht U').one).left
                    ≫ (_pb (cofinalProjSystem.proj hbN)
                        (baseChangeObj (cofinalProjSystem.proj b.2.1) (T.ht U').one)).cone.π₁
                      ≫ z₀.left ≫ (_pb (cofinalProjSystem.proj b.2.2) (L.F hUU' xE')).cone.π₁ := by
              have hcollapse := transApp_f_π₁π₁ cofinalProjSystem b.2.2 hbN (L.F hUU' xE')
                (𝟙 _)
              have hcollapse' :
                  (transApp L b.2.2 hbN (L.F hUU' xE')).left
                      ≫ (_pb (cofinalProjSystem.proj hbN)
                        (baseChangeObj (cofinalProjSystem.proj b.2.2) (L.F hUU' xE'))).cone.π₁
                      ≫ (_pb (cofinalProjSystem.proj b.2.2) (L.F hUU' xE')).cone.π₁ =
                    (_pb (cofinalProjSystem.proj ((wsDirected S).trans b.2.2 hbN))
                      (L.F hUU' xE')).cone.π₁ := by
                simpa only [Functor.id_obj, Cat.comp_id] using hcollapse
              have hp := congrArg
                (· ≫ (_pb (cofinalProjSystem.proj b.2.2) (L.F hUU' xE')).cone.π₁) hpush
              simp only [CategoryTheory.Category.assoc] at hp
              have hp' :
                  (pushHom L (T.ht U').one (L.F hUU' xE') b.2.1 b.2.2 hbN z₀).left
                      ≫ (_pb (cofinalProjSystem.proj ((wsDirected S).trans b.2.2 hbN))
                        (L.F hUU' xE')).cone.π₁ =
                    (transApp L b.2.1 hbN (T.ht U').one).left
                      ≫ (_pb (cofinalProjSystem.proj hbN)
                        (baseChangeObj (cofinalProjSystem.proj b.2.1) (T.ht U').one)).cone.π₁
                      ≫ z₀.left
                      ≫ (_pb (cofinalProjSystem.proj b.2.2) (L.F hUU' xE')).cone.π₁ := by
                calc
                  _ = (pushHom L (T.ht U').one (L.F hUU' xE') b.2.1 b.2.2 hbN z₀).left
                        ≫ ((transApp L b.2.2 hbN (L.F hUU' xE')).left
                          ≫ (_pb (cofinalProjSystem.proj hbN)
                            (baseChangeObj (cofinalProjSystem.proj b.2.2)
                              (L.F hUU' xE'))).cone.π₁
                          ≫ (_pb (cofinalProjSystem.proj b.2.2) (L.F hUU' xE')).cone.π₁) :=
                    congrArg (Cat.comp
                      (pushHom L (T.ht U').one (L.F hUU' xE') b.2.1 b.2.2 hbN z₀).left)
                      hcollapse'.symm
                  _ = _ := hp
              change (pushHom L (T.ht U').one (L.F hUU' xE') b.2.1 b.2.2 hbN z₀).left
                  ≫ (_pb (cofinalProjSystem.proj ((wsDirected S).trans b.2.2 hbN))
                    (L.F hUU' xE')).cone.π₁ = _
              exact hp'
            ...
            --   `hrec` reduces the LHS deep source-content to `transApp(one).left ≫ π₁ ≫ z₀.left ≫
            --   π₁(proj b.2.2, L.F hUU' xE')`; the goal still needs `… ≫ m.left ≫ fst`.  Close by
            --   projecting `hstage` (`zN ⊚ pushHom pf₀ = pushHom sc₀`) to its `.left` deep A-coordinate:
            --     • RHS leg `pushHom sc₀`: `proj_pushHom_f_π₁` (= `hscPush`) + `transApp_f_π₁π₁` gives
            --       `sc₀`'s deep content; `sc₀ = reflApp ≫ (sfp ⊚ cod) ≫ isoInv`, and `(sfp ⊚ cod).left
            --       ≫ fst = (sliceFactorPoint A fst).left ≫ fst = fst` (via `bcSliceIso`/`pIso`/`cod`
            --       content laws on the A-coordinate);
            --     • LHS leg `zN ⊚ pushHom pf₀`: `pf₀ = reflApp ≫ pushFibre g'' ≫ isoInv`, `pushFibre =
            --       Functor.map g''`, content `baseChangeMap_f_π₁` = `g''.left`, and `m.left = (g'' ⊚ pInv).left
            --       = g''.left ≫ pInv.left`, so the A-coordinate of `pf₀`-pushed `zN` is `z₀`-content `≫ m.left
            ...
            -- cast `(hcodObj ▸ zN).left` does NOT collapse via `eqToHom_refl`/`Subsingleton.elim`/`rfl`
            ...
            -- push: `(sliceFactorPoint A fst).left ≫ fst = fst` (`pair`/`fst_pair`); `bcSliceIso`'s A-leg
            -- `bcSliceIso.left ≫ (_pb snd (sliceEmbedObj P A)).π₁ = pair fst (snd≫snd)` (`lift_fst`), so
            ...
            -- `π₁(snd)`; then `hrec` to expose `z₀.left`; then project `hstage` (`proj_pushHom_f_π₁` on
            -- `pushHom pf₀`, with `pf₀ = reflApp ≫ Functor.map g'' ≫ isoInv`, `baseChangeMap_f_π₁` =
            -- `g''.left`, `m.left = g''.left ≫ pInv.left`) against the `sc₀` RHS (content `fst` via the verified
            ...
            -- project `hstage`'s `.left` onto the codomain content leg (matching `hscPush`).
            have hstageProj := congrArg (fun w => CategoryTheory.CommaMorphism.left w
                ≫ (transApp L ((wsDirected S).refl U') hUN'
                      (L.F hUU' (L.F hbU (terminalSliceObj W A)))).left
            ...
            rw [show (pushHom L (T.ht U').one (L.F hUU' xE') b.2.1 b.2.2 hbN z₀
                    ≫ pushHom L (L.F hUU' xE') (L.F hUU' (L.F hbU (terminalSliceObj W A)))
                        ((wsDirected S).refl U') ((wsDirected S).refl U') hUN pf₀).left
                  = (pushHom L (T.ht U').one (L.F hUU' xE') b.2.1 b.2.2 hbN z₀).left
                    ≫ (pushHom L (L.F hUU' xE') (L.F hUU' (L.F hbU (terminalSliceObj W A)))
                        ((wsDirected S).refl U') ((wsDirected S).refl U') hUN pf₀).left from rfl] at hstageProj
            simp only [CategoryTheory.Category.assoc] at hstageProj
            rw [hscPush, hpfPush] at hstageProj
            -- `hstageProj` now reads (a clean content equation over `S`):
            --   (pushHom z₀).left ≫ transApp(hUN,xE').left ≫ π₁(proj hUN, L.F hUU' xE') ≫ pf₀.left
            --     = transApp(hUN',one).left ≫ π₁(proj hUN', one) ≫ sc₀.left
            -- where `π₁ ≫ pf₀.left`/`π₁ ≫ sc₀.left` reach the slice-domain pullbacks (NOT `prod A P`
            ...
            let Λ : (L.F hUU' (L.F hbU (terminalSliceObj W A))).left ⟶ A :=
              (eqToHom hcodEq).left
                ≫ (_pb (snd : prod A P ⟶ P) (L.F hbU (terminalSliceObj W A))).cone.π₁
                  ≫ pInv.left ≫ (fst : prod A P ⟶ A)
            have gen' : ∀ {C : S} {Z Y Y' : Over C} (e : Y = Y') (f : OverHom Z Y),
                (e ▸ f).left = f.left ≫ (eqToHom e).left := by
              intro C Z Y Y' e f; subst e
              rw [eqToHom_refl]
              show f.left = f.left ≫ 𝟙 Y.left
              rw [CategoryTheory.Category.comp_id]
            have hcodf : cod.left = cod'.left ≫ (eqToHom hcodEq.symm).left := gen' hcodEq.symm cod'
            -- the two `eqToHom hcodEq.symm`/`hcodEq` cancel.
            have hee : (eqToHom hcodEq.symm).left
                  ≫ (eqToHom hcodEq).left
                = 𝟙 _ :=
              congrArg CategoryTheory.CommaMorphism.left (eqToHom_symm_comp_eqToHom hcodEq)
            -- `cod`'s A-leg is `fst`: chain `bcSliceIso`/`baseChangeMap pIso`/`pInv`.
            have hcod_A : cod.left ≫ Λ = (fst : prod A (prod A P) ⟶ A) := by
              show cod.left ≫ (eqToHom hcodEq).left
                  ≫ (_pb (snd : prod A P ⟶ P) (L.F hbU (terminalSliceObj W A))).cone.π₁
                    ≫ pInv.left ≫ (fst : prod A P ⟶ A) = _
              rw [hcodf]
              -- collapse the eqToHom pair.
              rw [CategoryTheory.Category.assoc, ← CategoryTheory.Category.assoc (eqToHom hcodEq.symm).left, hee, CategoryTheory.Category.id_comp]
              -- `cod'.left = bcSliceIso.left ≫ (baseChangeMap snd pIso).left`.
              show ((bcSliceIso A P).left
                  ≫ (@Functor.map _ _ _ _ _ (baseChangeFunctor (snd : prod A P ⟶ P)) _ _ pIso).left)
                  ≫ (_pb (snd : prod A P ⟶ P) (L.F hbU (terminalSliceObj W A))).cone.π₁
                    ≫ pInv.left ≫ (fst : prod A P ⟶ A) = _
              rw [CategoryTheory.Category.assoc,
                  ...
                  ← CategoryTheory.Category.assoc (baseChangeMap (snd : prod A P ⟶ P) pIso).left,
                  baseChangeMap_f_π₁ (snd : prod A P ⟶ P) pIso]
              simp only [CategoryTheory.Category.assoc]
              rw [show pIso.left ≫ pInv.left ≫ (fst : prod A P ⟶ A) = (fst : prod A P ⟶ A) from by
                    rw [← CategoryTheory.Category.assoc, show pIso.left ≫ pInv.left = 𝟙 (prod A P) from
                      congrArg CategoryTheory.CommaMorphism.left hp1]; exact CategoryTheory.Category.id_comp _,
                  ← CategoryTheory.Category.assoc,
                  show (bcSliceIso A P).left
            ...
            let Λr : (L.F ((wsDirected S).refl U')
                (L.F hUU' (L.F hbU (terminalSliceObj W A)))).left ⟶ A :=
              (reflApp L (L.F hUU' (L.F hbU (terminalSliceObj W A)))).left ≫ Λ
            -- `sfp.left ≫ fst = fst`.
            have hsfp_A : sfp.left ≫ (fst : prod A (prod A P) ⟶ A) = (fst : prod A P ⟶ A) := by
              show pair (fst : prod A P ⟶ A) (𝟙 (prod A P)) ≫ (fst : prod A (prod A P) ⟶ A) = _
              exact fst_pair _ _
            -- PIECE 1a: `sc₀.left ≫ Λr = (reflApp L one).left ≫ fst`.
            have hsc_A : sc₀.left ≫ Λr
                = (reflApp L (T.ht U').one).left ≫ (fst : prod A P ⟶ A) := by
              show ((reflApp L (T.ht U').one).left ≫ (sfp ⊚ cod).left
                  ≫ (isoInv (reflApp_isIso L (L.F hUU' (L.F hbU (terminalSliceObj W A))))).left)
                  ≫ (reflApp L (L.F hUU' (L.F hbU (terminalSliceObj W A)))).left ≫ Λ = _
              rw [CategoryTheory.Category.assoc, CategoryTheory.Category.assoc,
                  ← CategoryTheory.Category.assoc (isoInv (reflApp_isIso L (L.F hUU' (L.F hbU (terminalSliceObj W A))))).left,
                  show (isoInv (reflApp_isIso L (L.F hUU' (L.F hbU (terminalSliceObj W A))))).left
                      ≫ (reflApp L (L.F hUU' (L.F hbU (terminalSliceObj W A)))).left
                    = 𝟙 _ from
                  congrArg CategoryTheory.CommaMorphism.left
                    (inv_isoInv_comp (reflApp_isIso L (L.F hUU' (L.F hbU (terminalSliceObj W A))))),
                  CategoryTheory.Category.id_comp,
                  show (sfp ⊚ cod).left = sfp.left ≫ cod.left from rfl, CategoryTheory.Category.assoc, hcod_A, hsfp_A]
            -- PIECE 1b: `(pushFibre g'').left ≫ Λ`.  `pushFibre g'' = Functor.map (functF hUU') g''
            -- = baseChangeMap (selectProj U' U hUU') g''`; with `selectProj U' U hUU' = snd` and the
            -- eqToHom casts (`hLF`/`hcodEq`), its A-leg is `(eqToHom hLF).left ≫ π₁(snd, xE') ≫ g''.left`.
            ...
            have bcMap_cast : ∀ {C E : S} {a b : E ⟶ C} (e : a = b) {X Y : Over C} (gg : OverHom X Y),
                (baseChangeMap a gg).left ≫ (eqToHom (congrArg (fun z => baseChangeObj z Y) e)).left
                  = (eqToHom (congrArg (fun z => baseChangeObj z X) e)).left ≫ (baseChangeMap b gg).left := by
              intro C E a b e X Y gg; subst e
              rw [eqToHom_refl, eqToHom_refl]
              show (baseChangeMap a gg).left ≫ 𝟙 _ = 𝟙 _ ≫ (baseChangeMap a gg).left
              rw [CategoryTheory.Category.comp_id, CategoryTheory.Category.id_comp]
            have hpushFibre_A : (pushFibre W A hbU hUU' g'').left ≫ Λ
                = (eqToHom hLF).left ≫ (_pb (snd : prod A P ⟶ P) xE').cone.π₁ ≫ m.left ≫ (fst : prod A P ⟶ A) := by
              show (baseChangeMap (tSelectProj Prod.snd U'.val U.val hUU') g'').left
                  ≫ (eqToHom hcodEq).left ≫ (_pb (snd : prod A P ⟶ P) (L.F hbU (terminalSliceObj W A))).cone.π₁
                    ≫ pInv.left ≫ (fst : prod A P ⟶ A) = _
              rw [← CategoryTheory.Category.assoc,
                  show (eqToHom hcodEq)
                  ...
                  CategoryTheory.Category.assoc, ← CategoryTheory.Category.assoc (baseChangeMap (snd : prod A P ⟶ P) g'').left,
                  baseChangeMap_f_π₁ (snd : prod A P ⟶ P) g'']
              -- `g''.left ≫ pInv.left = m.left`.
              simp only [CategoryTheory.Category.assoc]
              rw [show g''.left ≫ pInv.left ≫ (fst : prod A P ⟶ A) = m.left ≫ (fst : prod A P ⟶ A) from by
                    rw [← CategoryTheory.Category.assoc]; rfl]
            -- PIECE 1b (assembled): `pf₀.left ≫ Λr`.
            have hpf_A : pf₀.left ≫ Λr
                = (reflApp L (L.F hUU' xE')).left
                    ≫ (eqToHom hLF).left ≫ (_pb (snd : prod A P ⟶ P) xE').cone.π₁
                      ≫ m.left ≫ (fst : prod A P ⟶ A) := by
              show ((reflApp L (L.F hUU' xE')).left ≫ (pushFibre W A hbU hUU' g'').left
                  ≫ (isoInv (reflApp_isIso L (L.F hUU' (L.F hbU (terminalSliceObj W A))))).left)
                  ≫ (reflApp L (L.F hUU' (L.F hbU (terminalSliceObj W A)))).left ≫ Λ = _
              rw [CategoryTheory.Category.assoc, CategoryTheory.Category.assoc,
                  ← CategoryTheory.Category.assoc (isoInv (reflApp_isIso L (L.F hUU' (L.F hbU (terminalSliceObj W A))))).left,
                  ...
                  congrArg CategoryTheory.CommaMorphism.left
                    (inv_isoInv_comp (reflApp_isIso L (L.F hUU' (L.F hbU (terminalSliceObj W A))))),
                  CategoryTheory.Category.id_comp, hpushFibre_A]
            -- ── PIECE 2: cast-eliminate the goal LHS down to `zN.left ≫ π₁(inner)`. ──
            -- collapse `(hcodObj ▸ zN).left ≫ πOut ≫ πIn` to `zN.left ≫ π₁(proj(trans b.2.2 hbN), L.F hUU' xE')`.
            have hLHScollapse : (hcodObj ▸ zN).left
              ...
              rw [hcastf, CategoryTheory.Category.assoc,
                  ← CategoryTheory.Category.assoc (eqToHom hcodObj).left, hcodObj', hbcInner hLF, CategoryTheory.Category.assoc,
            ...
            simp only [CategoryTheory.Category.assoc] at hSP
            rw [hpf_A, hsc_A] at hSP
            -- rewrite the goal LHS via `hLHScollapse` (with the `≫ m.left ≫ fst` tail).
            have hLHSfull : (hcodObj ▸ zN).left
                  ≫ ((_pb (tSelectProj Prod.snd N.val U'.val hUN') (baseChangeObj (snd : prod A P ⟶ P) xE')).cone.π₁
                      ≫ (_pb (snd : prod A P ⟶ P) xE').cone.π₁) ≫ m.left ≫ (fst : prod A P ⟶ A)
                = zN.left
                  ≫ ((_pb (tSelectProj Prod.snd N.val U'.val hUN') (L.F hUU' xE')).cone.π₁
                      ≫ (_pb (tSelectProj Prod.snd U'.val U.val hUU') xE').cone.π₁) ≫ m.left ≫ (fst : prod A P ⟶ A) := by
              rw [← CategoryTheory.Category.assoc, ← CategoryTheory.Category.assoc, hLHScollapse, CategoryTheory.Category.assoc, CategoryTheory.Category.assoc]
            rw [hLHSfull]
            -- ── ISOLATED RESIDUAL (★ + Piece 3).  Goal now:
            --   zN.left ≫ (πOut' ≫ πIn') ≫ m.left ≫ fst = srcPB.cone.π₂ ≫ ψ ≫ fst.
            -- `hSP` (post-composed `hstageProj` with `Λr`, Pieces 1a/1b applied) reads:
            --   zN.left ≫ (transApp(refl U',hUN) xE').left ≫ π₁ ≫ (reflApp (L.F hUU' xE')).left
            --        ≫ (eqToHom hLF).left ≫ πIn ≫ m.left ≫ fst
            --     = (transApp(refl U',hUN') one).left ≫ π₁ ≫ (reflApp one).left ≫ fst.
            -- TWO remaining micro-steps, both pullback-pasting reindexings:
            --  (★)  πOut' ≫ πIn' = (transApp(refl U',hUN) xE').left ≫ π₁ ≫ (reflApp (L.F hUU' xE')).left
            --         ≫ (eqToHom hLF).left ≫ πIn  — identifies the descent-domain projection
            ...
            --       `z := (eqToHom hLF).left ≫ πIn` collapses the `transApp ≫ π₁ ≫ (inner π₁) ≫ z` to
            --       `(_pb (proj (trans (refl U') hUN)) (L.F hUU' xE')).cone.π₁ ≫ z = πOut' ≫ z`; then
            --       `eqToHom_bc_π₁ hsp xE'` turns `z = (eqToHom hLF).left ≫ πIn` into `πIn'`.  The ONLY
            --       gap is identifying `(reflApp (L.F hUU' xE')).left` (the explicit `pf₀` unit) with the
            ...
            --  (Piece 3)  `hSP`'s RHS `(transApp(refl U',hUN') one).left ≫ π₁ ≫ (reflApp one).left ≫ fst`
            ...
            -- ── (★): collapse `transApp ≫ π₁ ≫ reflApp ≫ (eqToHom hLF).left ≫ π₁(snd)` in hSP's LHS to
            ...
            -- LHS: `transApp(refl U',hUN) xE').left ≫ π₁(proj hUN, bc) ≫ π₁(proj(refl U'),xE') ≫ z`,
            -- with `z := (eqToHom hLF).left ≫ π₁(snd,xE') ≫ m.left ≫ fst`.
            rw [transApp_f_π₁π₁ cofinalProjSystem ((wsDirected S).refl U') hUN (L.F hUU' xE')
                  ((eqToHom hLF).left ≫ (_pb (snd : prod A P ⟶ P) xE').cone.π₁
                    ≫ m.left ≫ (fst : prod A P ⟶ A))] at hSP
            -- RHS: collapse with `z := fst`.
            rw [transApp_f_π₁π₁ cofinalProjSystem ((wsDirected S).refl U') hUN' HasTerminal.one
                  (fst : prod A P ⟶ A)] at hSP
            -- collapse `(eqToHom hLF).left ≫ π₁(snd)` to `π₁_U'`.
            have hcollapse0 : (eqToHom hLF).left ≫ (_pb (snd : prod A P ⟶ P) xE').cone.π₁
                = (_pb (tSelectProj Prod.snd U'.val U.val hUU') xE').cone.π₁ := by
              rw [hLF']; exact eqToHom_bc_π₁ hsp xE'
            have hcollapseU' : (eqToHom hLF).left ≫ (_pb (snd : prod A P ⟶ P) xE').cone.π₁
                  ≫ m.left ≫ (fst : prod A P ⟶ A)
                = (_pb (tSelectProj Prod.snd U'.val U.val hUU') xE').cone.π₁ ≫ m.left ≫ (fst : prod A P ⟶ A) := by
              rw [← CategoryTheory.Category.assoc, hcollapse0]
            rw [hcollapseU'] at hSP
            -- hSP now: `zN.left ≫ π₁_N ≫ π₁_U' ≫ m.left ≫ fst = srcPB.π₁ ≫ fst`.  Match goal LHS via assoc.
            rw [show (_pb (cofinalProjSystem.proj ((wsDirected S).trans ((wsDirected S).refl U') hUN))
                        (L.F hUU' xE')).cone.π₁
                      ≫ (_pb (tSelectProj Prod.snd U'.val U.val hUU') xE').cone.π₁ ≫ m.left ≫ (fst : prod A P ⟶ A)
                  = ((_pb (cofinalProjSystem.proj ((wsDirected S).trans ((wsDirected S).refl U') hUN))
                        (L.F hUU' xE')).cone.π₁
                      ≫ (_pb (tSelectProj Prod.snd U'.val U.val hUU') xE').cone.π₁) ≫ m.left ≫ (fst : prod A P ⟶ A)
                from (CategoryTheory.Category.assoc _ _ _).symm] at hSP
            rw [hSP]
            ...
            have hsrcW : srcPB.cone.π₁ = srcPB.cone.π₂ ≫ tSelectProj Prod.snd N.val U'.val hUN' :=
              (CategoryTheory.Category.comp_id srcPB.cone.π₁).symm.trans srcPB.cone.w
            ...
            rw [hsrcW, CategoryTheory.Category.assoc, hfstFactor]
          rw [show (r ≫ (zNd.left ≫ codPB.cone.π₁)) ≫ cnDN.π₁ ≫ mC.left ≫ (fst : prod A PN ⟶ A)
                = r ≫ ((zNd.left ≫ codPB.cone.π₁) ≫ cnDN.π₁ ≫ mC.left ≫ (fst : prod A PN ⟶ A)) from
              CategoryTheory.Category.assoc _ _ _, hbridge]
          -- `r ≫ srcPB.cone.π₂ = a = ψ⁻¹`, and `ψ⁻¹ ≫ ψ ≫ fst = fst`.
          rw [show r ≫ srcPB.cone.π₂ ≫ ψ ≫ (fst : prod A PN ⟶ A)
                = (r ≫ srcPB.cone.π₂) ≫ ψ ≫ (fst : prod A PN ⟶ A) from (CategoryTheory.Category.assoc _ _ _).symm,
              srcPB.lift_snd srcCone]
          show isoInv hψiso ≫ ψ ≫ (fst : prod A PN ⟶ A) = fst
          rw [show isoInv hψiso ≫ ψ ≫ (fst : prod A PN ⟶ A)
                = (isoInv hψiso ≫ ψ) ≫ (fst : prod A PN ⟶ A) from (CategoryTheory.Category.assoc _ _ _).symm,
              inv_isoInv_comp hψiso, CategoryTheory.Category.id_comp]
    ...
    have hsq : q ≫ Dbar.hom = (𝟙 (prod A PN)) ≫ (snd : prod A PN ⟶ PN) := by
      rw [CategoryTheory.Category.id_comp]; exact hqstruct
    obtain ⟨u, ⟨hu₁, hu₂⟩, _⟩ := hcnDN ⟨prod A PN, q, 𝟙 (prod A PN), hsq⟩
    -- `t.left := u`; over-hom law `u ≫ cnDN.π₂ = id` is `hu₂`.
    refine ⟨CategoryTheory.Over.homMk u ?_, ?_⟩
    · show u ≫ cnDN.π₂ = (overTerm (prod A PN)).hom
      show u ≫ cnDN.π₂ = 𝟙 (prod A PN)
      exact hu₂
    · -- `t ⊚ mbarN = sliceFactorPoint A fst`: check underlying `u ≫ mf'N = pair fst id` by the eta law.
      apply CategoryTheory.Over.OverMorphism.ext
      show u ≫ mf'N = pair (fst : prod A PN ⟶ A) (𝟙 (prod A PN))
      rw [pair_uniq _ _ (u ≫ mf'N) rfl rfl]
      congr 1
      · -- `A`-leg: `(u ≫ mf'N) ≫ fst = fst`.
        rw [CategoryTheory.Category.assoc, hmf1N, ← CategoryTheory.Category.assoc, hu₁]; exact hqfresh
      · -- `PN`-leg: `(u ≫ mf'N) ≫ snd = id`.
        rw [CategoryTheory.Category.assoc, hmf2N]; exact hu₂
  obtain ⟨t, hfac⟩ := hpt
