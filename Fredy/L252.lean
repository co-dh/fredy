/-
  ...
  constraint `0 ≤ starti < endi`, the same STRICT validity as `Fredy/L435.lean`), decide whether a
  ...
  Same recipe as `Fredy/L56.lean` (Merge Intervals) and `Fredy/L435.lean` (Non-overlapping
  Intervals): sort by START (`Fredy.L56`'s `isort`/`Sorted`, reused verbatim — no re-derivation),
  ...
  3. **Specification** — the honest all-pairs one, `Fredy.L435`-style: `NonOverlap := List.Pairwise
  ...
  Mathlib-free (Lean core `Init` — `List.Pairwise`, `List.Perm` — plus `Fredy.L56`, itself only
