/-
  ...
  `Fredy/L543.lean` WRITES the pair-carrying fold `foldFn = (height, diam)` by hand and then verifies
...
/-! ## Running / cross-checking the emergent fold against `Fredy/L543.lean`
...
/-- The derived answer agrees with `L543.solveFn` on every example tree of `Fredy/L543.lean`. -/
