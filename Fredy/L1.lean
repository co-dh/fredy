/-
  ...
  `nums[i] + nums[j] = target`.  Unlike `Fredy/L217.lean` (a Bool DECISION), this is a
  ...
  | cons x xs ih =>
    ...
    · -- `findComplement` missed: recurse with the new element pushed onto `seen`.
      ...
      have hnew : ∀ (v : Int) (i : Nat), done[i]? = some v → v ≠ target - x := by
        intro v i hdi heq
        subst heq
        exact findComplement_none hfc i ((hseen (target - x) i).mpr hdi)
      have hlen' : ((x, seen.length) :: seen).length = (done ++ [x]).length := by
        simp [hlen]
      have hseen' : SeenIff (done ++ [x]) ((x, seen.length) :: seen) := by
        intro v i
        constructor
        · intro hmem
        ...
        · intro hdi'
          ...
          · have hieq : i = done.length := by
            ...
            rw [← hxv, hiseen]
            exact List.mem_cons_self ..
      have hnp' : ∀ i j, ¬ TwoSum (done ++ [x]) target i j := by
      ...
      have := ih (done ++ [x]) ((x, seen.length) :: seen) target hlen' hseen' hnp'
