/-
  ...
  `Fredy/L20.lean` writes the depth-scan fold `foldFn : SnocList Bool Bool → Int × Bool` by hand
  ...
  `Freyd.Alg.RelSet.SL.snocFold_unique` (`Fredy/A6_GenFold.lean`), which — unlike the product-only
  ...
  `foldFn`'s own defining equations (`Fredy/L20.lean`, `foldFn_wrap`/`foldFn_snoc_true`/
...
/-! ## Running / cross-checking the emergent fold against `Fredy/L20.lean` -/
