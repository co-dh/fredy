...
theorem isHom_id (A : AbelianGroupObject 𝒞) :
    IsHomAbelianGroupObject A A (𝟙 A.carrier) := by
  unfold IsHomAbelianGroupObject
  -- A.add ≫ id = A.add ;  ⟨fst ≫ id, snd ≫ id⟩ ≫ A.add = ⟨fst, snd⟩ ≫ A.add = id ≫ A.add = A.add
  rw [CategoryTheory.Category.comp_id, CategoryTheory.Category.comp_id, CategoryTheory.Category.comp_id, pair_fst_snd, CategoryTheory.Category.id_comp]
...
theorem isHom_comp {A B C : AbelianGroupObject 𝒞}
  ...
  rw [← CategoryTheory.Category.assoc, hx, CategoryTheory.Category.assoc, hy, ← CategoryTheory.Category.assoc]
  ...
  have : (pair (fst ≫ x) (snd ≫ x)) ≫ (pair (fst ≫ y) (snd ≫ y))
    ...
    unfold abSq at this
    simpa [CategoryTheory.Category.assoc] using this
  rw [this]
...
instance instCatAb : CategoryTheory.Category.{v} (AbelianGroupObject 𝒞) where
  Hom A B := HomAb A B
  id A := ⟨𝟙 A.carrier, isHom_id A⟩
  comp f g := ⟨f.val ≫ g.val, isHom_comp f.property g.property⟩
  id_comp f := Subtype.ext (CategoryTheory.Category.id_comp f.val)
  comp_id f := Subtype.ext (CategoryTheory.Category.comp_id f.val)
  assoc f g h := Subtype.ext (CategoryTheory.Category.assoc f.val g.val h.val)
...
@[simp] theorem ab_id_val (A : AbelianGroupObject 𝒞) :
    (show HomAb A A from 𝟙 A).val = 𝟙 A.carrier := rfl
...
theorem zero_add (f : T ⟶ B.carrier) :
  ...
  simp only at h
  rw [← CategoryTheory.Category.assoc, ab_pair_precomp] at h
  -- f ≫ (term ≫ zero) = term_T ≫ zero  and  f ≫ id = f
  rwa [CategoryTheory.Category.comp_id, ← CategoryTheory.Category.assoc, term_uniq (f ≫ term B.carrier) (term T)] at h
...
theorem add_comm (f g : T ⟶ B.carrier) :
  ...
  simp only at h
  rw [← CategoryTheory.Category.assoc, ab_pair_precomp, snd_pair, fst_pair] at h
  exact h.symm
...
theorem neg_add (f : T ⟶ B.carrier) :
  ...
  simp only at h
  rw [← CategoryTheory.Category.assoc, ab_pair_precomp, CategoryTheory.Category.comp_id, ← CategoryTheory.Category.assoc,
...
theorem add_assoc (f g h : T ⟶ B.carrier) :
  ...
  simp only [← CategoryTheory.Category.assoc, fst_pair, snd_pair, ab_pair_precomp] at h0
  exact h0
...
theorem comp_add {S : 𝒞} (k : S ⟶ T) (f g : T ⟶ B.carrier) :
  ...
  rw [← ab_pair_precomp, CategoryTheory.Category.assoc]
...
theorem isHom_addCar (x y : HomAb A B) :
  ...
  rw [← CategoryTheory.Category.assoc, ab_pair_precomp, x.property, y.property]
  rw [show pair (fst ≫ (pair x.val y.val ≫ B.add)) (snd ≫ (pair x.val y.val ≫ B.add))
      ...
      rw [← CategoryTheory.Category.assoc, ab_pair_precomp, ← CategoryTheory.Category.assoc, ab_pair_precomp]]
...
theorem isHom_zeroCar (A B : AbelianGroupObject 𝒞) :
  ...
  rw [← CategoryTheory.Category.assoc, term_uniq (A.add ≫ term A.carrier) (term (prod A.carrier A.carrier)),
      ← CategoryTheory.Category.assoc, term_uniq (fst ≫ term A.carrier) (term (prod A.carrier A.carrier)),
      ← CategoryTheory.Category.assoc, term_uniq (snd ≫ term A.carrier) (term (prod A.carrier A.carrier))]
...
theorem isHom_negCar (x : HomAb A B) :
  ...
  rw [← CategoryTheory.Category.assoc, x.property, ← CategoryTheory.Category.assoc,
      ← GElt.neg_add_distrib B (fst ≫ x.val) (snd ≫ x.val), CategoryTheory.Category.assoc, CategoryTheory.Category.assoc]
...
theorem addCar_comp (x y : HomAb A B) (z : HomAb B C) :
  ...
  --                   = ⟨x≫z, y≫z⟩ ≫ C.add.
  rw [CategoryTheory.Category.assoc, z.property, ← CategoryTheory.Category.assoc, ab_pair_precomp]
