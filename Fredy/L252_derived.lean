/-
  ...
  `Fredy/L252.lean` sorts by start (`Fredy.L56.isort`, reused verbatim — the sort is NOT re-derived
  ...
  (`Fredy/A6_GenFold.lean`) over `ConsList Unit (Int×Int)` (`Fredy/A6_ConsList.lean`, the initial
  ...
  Raw `List` reshaping (the recipe of `Fredy/A6_GenFold.lean`'s header): `noAdjFromC` is folded over
  ...
  scan from the sorted list's own head — `Fredy.L252`'s own thin wrapper, not part of the scan) and
  ...
  **Complexity**: the emergent scan is a single structural pass over the (sorted) list, O(n); the
  ...
  left as-is per `Fredy/L252.lean`'s own docstring, not re-derived).

  Mathlib-free (Lean core `Init` plus `Fredy.A6_GenFold`, `Fredy.A6_ConsList`, `Fredy.L252`).  We
...
/-- **The derived program**: sort (unchanged, `Fredy.L56.isort`), then the emergent scan. -/
