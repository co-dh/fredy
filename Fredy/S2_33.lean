/-
  ...
  through two pieces of data, which we take as EXPLICIT hypotheses:
    ...
    • Moerdijk's embedding, supplied as a FAITHFUL `AllegoryFunctor` from the §2.218 target
      ...
      (`OSetFrameHom.functor`, built below from `Fredy.Locale`); the topological theorem that the
...
/-! ## §2.227/§2.331  The frame-reindexing allegory functor `OSet(F) ⟶ OSet(G)`
  ...
  `Fredy.Locale` builds `OSetFrameHom.{obj, map, map_comp, map_id, map_recip, map_inter}` from a
...
/-! ## §2.331 (iii)/(iv) — logos and coprime terminator
  ...
  `Fredy/Locale.lean`).  Part (iv) ADDS a *coprime terminator* hypothesis (a focal/atomic point of
