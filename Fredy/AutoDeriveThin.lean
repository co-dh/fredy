/-
  ...
  B&dM's THEOREM 8.1 / Corollary 8.1 (`Fredy.A8_1.thinning` / `thinning_min`) refine an
  ...
  * `correct_value` — the scalar-answer packaging via a problem-specific spec
  ...
  binary-thinning example, whose concrete program `Fredy.A8_4_Knapsack` had deferred.
...
/-! ## Demo: 0/1 knapsack (B&dM §8.4 — the book's binary-thinning example), concretely
  ...
  `Fredy.A8_4_Knapsack` states the §8.4 refinement abstractly (an instance of `thinning_min`)
