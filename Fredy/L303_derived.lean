/-
  ...
  `Fredy/L303.lean` WRITES `scanlAdd a (x :: xs) = a :: scanlAdd (a + x) xs`, `scanlAdd a [] = [a]`
  ...
  (`CL.ofList`, `Fredy/A6_ConsList.lean`, front-to-back: `[] ↦ wrap ()`, `x :: xs ↦ cons x
  (ofList xs)`) and produces it as the catamorphism of an EMERGENT algebra via the general-carrier
  law `CL.consFold_unique` (`Fredy/A6_GenFold.lean`) — the exact same CPS-carrier shape as
  `Fredy/L238_derived.lean`'s prefix-product scan `preScan`:
