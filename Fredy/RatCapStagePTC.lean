...
/-\! ### 2. Mono preservation — `g*` is a right adjoint (`Σ_g ⊣ g*`)
...
theorem projStage_preservesMono {i j : ι} (hij : D.le i j)
    ...
    (hφ : Mono φ) :
    Mono (@Functor.map _ _ _ _ _ ((laxOfProjSystem' P).functF hij) x y φ) := by
...
/-\! ### 1. Conservativity — `g*` reflects isos AMONG MONOS (given `g` a cover)
  ...
  reflects isos among monos.  The fibre `IsIso`/`Mono` are definitionally the slice `OverIso`/
...
theorem projStage_conservative [PreRegularCategory 𝒞] {i j : ι} (hij : D.le i j)
    ...
    {x y : (laxOfProjSystem' P).A i} (φ : x ⟶ y) (hφmono : Mono φ)
