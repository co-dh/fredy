...
theorem cover_comp_iso {𝒞 : Type u} [Cat.{u} 𝒞] {X Y Z : 𝒞} {f : X ⟶ Y} {e : Y ⟶ Z}
...
theorem wellSupported_step {S : Type u} [Cat.{u} S] [PreRegularCategory S]
  ...
  rw [hterm_eq]; exact cover_comp_iso hcov hiso
...
theorem stageRelCap_succ_of_wellPointed (b : PreRegBundle.{u}) (ccs : CofinalCapStep.{u})
    ...
    (hmono : @Mono _ (stageBundle ccs.step b j).cat _ _ m')
  ...
  have hsm : @Mono _ (ccs.step (stageBundle ccs.step b j)).catT _ _
