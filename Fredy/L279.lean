/-
  ...
  `v` — SOLVED THROUGH THE GENERIC DP DRIVER `Fredy.AutoDeriveDP` (the reusability check of
  auto-derive increment 2: a second, fresh instantiation, never hand-verified elsewhere).

  Same abstract skeleton as Coin Change (`Fredy.L322_dp`): a step-COUNTING DP over the amount
  ...
  * the `DPCount` bundle `sqDP` wiring them up.
  ...
  `omin` and its lemmas are reused from `Fredy.L322` (their current home).
