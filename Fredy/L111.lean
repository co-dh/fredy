/-
  ...
  children).  Mirrors `Fredy/L104.lean` (Maximum Depth) with `imax` swapped for `imin` — but with
  ...
  1. **Data** — `Tree Int` (`Fredy.A6_TreeBin`), `nil` an empty leaf slot, `node l a r` an
