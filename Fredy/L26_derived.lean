/-
  ...
  `Fredy/L26.lean` WRITES `dedupFn` as a two-element-LOOKAHEAD structural recursion over a RAW Lean
  `List Int`:
  ...
  WITHOUT tupling an extra lookahead component (contrast `Fredy/L13_derived.lean`'s
  ...
  (`Fredy/A6_GenFold.lean`) PRODUCES the dedup fold as `cataR (consScalarAlg g st)` — the
  ...
  `Freyd.Alg.RelSet.CL.ofList` (`Fredy/A6_ConsList.lean`), not a local redefinition.
