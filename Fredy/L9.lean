/-
  ...
  Same DECISION-problem recipe as `Fredy/L217.lean`/`Fredy/L125.lean` (`Fredy/leetcode.md` S5):
  ...
  4. **Specification and correctness.** `palin_correct` is the honest unfolding: `n` is a
     ...
     same way `Fredy/L125.lean`'s `IsPalin` is `toList xs = (toList xs).reverse` on an already-
     given sequence.
