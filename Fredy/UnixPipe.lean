/-
  ...
  executable finite allegory `FinRel` (`Fredy.RelInterp`), grounded in real Linux syscalls.

  A case study (like `Fredy.A6_1_Digits`), NOT a book section.  It replays the spec↔program
...
/-! ## Stage 3 — `sort`, PROGRAM side (polynomial): filter+insertion-sort FOLD over a plain List
  ...
  linsert e acc else acc`, `base = []` — the `Fredy.L56_derived` fold shape.  Generic in the
  keep-predicate and order so the fixture and the live `main` share one definition (DRY). -/

/-- Insertion into an ascending (by `le`) list — `Fredy.L56`'s `linsert`, generic in the order. -/
...
/-- **SPEC↔PROGRAM HEAD BRIDGE** (`eval` vs the program fold): the entry the exponential SPEC
    ...
    spec↔program agreement of `Fredy.L121`, now for a pipeline. -/
