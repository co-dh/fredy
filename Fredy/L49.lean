/-
  ...
  Same recipe as `Fredy/L242.lean` (Valid Anagram) — reuses its hard-won multiset machinery
  wholesale (`import Fredy.L242`):
  ...
  Mathlib-free (Lean core `Init` — `List.Pairwise`, `List.Perm` — plus `Fredy.L242`). Axioms
