/-
  ...
  This is a DECISION (`Bool`) built on `Fredy.A6_TreeBin`'s `Tree`, like `L20`/`L104`, but with a
  ...
  1. **Data** — `Tree Int` (`Fredy.A6_TreeBin`), `A = Int` (bounds are compared against labels).
