/-
  ...
  B&dM's Theorem 9.1 (`Fredy.A9_1.dynamic_programming`) models dynamic programming with the
  body `min R · P(h·FX) · ΛT°` (mirrored: `A (T°) ≫ powerRel (F.map X ≫ h) ≫ minRel R`).
  Its power relator is the EGLI–MILNER lifting (`Fredy.A5_4`): term₁ demands EVERY element of
  ...
  (`Fredy.L322`, comment at its end) — with coins {2,3}, amount 3 is solvable (one 3-coin),
  ...
  **Theorem `dynamic_programming_inf`**: if `h` is a map monotonic on the transitive `R`, the
  ...
  property argument (`Fredy.A9_1`).  Composition is diagram order (B&dM `X·Y` = Fredy
  `Y ≫ X`).  The instantiation re-deriving `Fredy.L322`'s `coinSpec` correctness from this
  theorem is `Fredy.L322_dp`.
-/
import Fredy.A9_1
...
/-- **Core of the ∞-DP theorem**: `M = min R · Λ(H ∪ τ)` is a PREFIXED point of `dpBodyInf`,
    ...
    `dp_prefixed` (`Fredy.A9_1`): the two obligations are the components of `min`'s universal
...
/-- **The ∞-completed dynamic-programming theorem** (new; strictly generalizes B&dM Theorem
    ...
    algebra `h` is STRICT at the adjoined top (`osucc ∞ = ∞` in `Fredy.L322`).  By
