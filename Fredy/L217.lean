/-
  ...
  Unlike `Fredy/L121.lean`/`Fredy/L53.lean` (an OPTIMUM `max(≤)·Λ spec`), this is a DECISION problem:
  the answer object is `Bool`, and correctness is a plain `iff`, `solveFn xs = true ↔ dupP xs`, not an
  extremum.  Same data/program recipe otherwise (`Fredy.A6_SnocList`, `Fredy/leetcode.md` skill S0):
