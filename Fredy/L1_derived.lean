/-
  ...
  `Fredy/L1.lean` WRITES `twoSumFn nums target = go [] target nums` as a LEFT-TO-RIGHT scan carrying an
  ...
  mathlib-free `Freyd.HashMap.AHashMap Nat` (value ↦ its index, `Fredy/A6_HashMap.lean`).  Each step
  ...
  `Freyd.Alg.RelSet.CL.consFold_unique` (`Fredy/A6_GenFold.lean`) PRODUCES the hash scan as
