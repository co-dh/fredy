/-
  ...
  `Fredy/L435.lean` writes `solveFn ivs = ivs.length - (keptSorted (isortH ivs)).length` as two
  ...
  `CL.consFold_unique` (`Fredy/A6_GenFold.lean`) — the recursions were never hand-written as folds.
  ...
  **Complexity**: `derivedSolve` is `O(n²)` overall, same as `LC435.solveFn` — dominated by
  ...
  Mathlib-free (Lean core + `Fredy.A6_GenFold`, `Fredy.A6_ConsList`, `Fredy.L435` only).  We route
