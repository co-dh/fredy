...
theorem prelogos_representation_theorem (A : Type u) [Cat.{u} A] [PositivePreLogos A] :
  ...
  -- BLOCKED (genuine). The book's proof chains four pieces, none yet built here:
  --   (1) Capital extension (§1.63): embed A into a capital positive pre-logos;
  --       depends on §1.633 (`capital_iff_complemented_subterminators`), itself blocked.
  --   (2) Complemented subterminators form a Boolean algebra (§1.631/§1.633 basis),
  --       which needs §1.631 projectivity — blocked on coproduct DISJOINTNESS (the
  --       `HasBinaryCoproducts` here carries no disjoint/universal axioms; see S1_64 note).
  --   (3) Stone representation of that Boolean algebra by its ultra-filters — the
  --       ultra-filter existence theorem is a CHOICE principle, which this project forbids
  --       unless unavoidable (CLAUDE.md). No constructive substitute is available.
  --   (4) The T_ℱ colimit-of-Hom-sets construction (§1.634) preserving the pre-logos
  --       structure — `prefilter_functor` here is only the raw pair-type, not the colimit.
  -- All four are unbuilt; (3) additionally requires choice. Cannot be closed faithfully.
  sorry
...
theorem capital_iff_complemented_subterminators [PositivePreLogos 𝒞] :
  ...
  -- BLOCKED (genuine), both directions:
  --   (⟹) Capital ⟹ complemented subterminators projective: this IS §1.631
  --       (`complemented_of_projective_is_projective`), which is itself blocked on
  --       coproduct DISJOINTNESS — its inner `hh : Cover h` needs the coproduct
  --       inclusions to be disjoint monos, but `HasBinaryCoproducts` here has no
  --       disjoint/universal axioms (confirmed by the S1_64 note). The "form a basis"
  --       conjunct further needs the §1.632 basis theory linking subterminators to
  --       jointly-faithful representables, which is not developed (only `IsBasis` is
  --       defined, with no constructions producing instances).
  --   (⟸) basis of projective complemented subterminators ⟹ Capital: reconstructs
  --       well-pointedness (§1.523/§1.525) from the basis covering proper monos; this
  --       requires turning a basis element G→A (G a subterminator) into a point 1→A,
  --       i.e. that subterminators are well-supported quotients of 1 — again unbuilt.
  -- Neither direction is reachable without the missing §1.631 disjoint-coproduct
  -- infrastructure and §1.632 basis constructions. Cannot be closed faithfully.
  sorry
