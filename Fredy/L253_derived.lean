/-
  ...
  `Fredy/L253.lean` computes `roomsFn ivs := (ivs.map (fun iv => countCover ivs iv.1)).foldr nmax 0`
  ...
  `Fredy/L1_derived.lean`'s hash scan, with carrier `C := Nat` in place of a continuation.
  ...
  3. **Correctness is REUSED**, not re-proved: `LC253.rooms_correct` (achievability `rooms_achievable`
     ...
     as the actual morphism equation `RelSet.eq_A_comp_maxRel` (`Fredy/A7_4_Horner.lean`) —
