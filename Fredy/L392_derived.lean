/-
  ...
  `Fredy/L392.lean` WRITES `isSubseqFn s t : List Int → List Int → Bool` by hand, structural on `t`
  ...
  (`Fredy.A6_ConsList.ofList`, front-to-back), and the AOP way to expose it is the general-carrier
  cons-list fold-uniqueness law `CL.consFold_unique` (`Fredy/A6_GenFold.lean`) — the same idea as
  ...
  `Fredy.L392`'s program; feeding it into `LC392.subseq_correct` gives the decision headline WITHOUT
...
/-! ## Running the emergent fold, cross-checked against `Fredy/L392.lean` -/
