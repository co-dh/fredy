/-
  ...
  Same recipe as `Fredy/L56.lean`/`Fredy/L242.lean` (`Fredy/leetcode.md` S0): the data object is a
  plain `List Int` (no `SnocList` needed — this is a one-pass structural transform, not a fold with
  auxiliary state). Reuses `LC242.Sorted : List Int → Prop` verbatim (`import Fredy.L242`), the DRY
