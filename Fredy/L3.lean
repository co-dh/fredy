/-
  ...
  Same recipe as `Fredy/L53.lean` / `Fredy/L300.lean` (see `Fredy/leetcode.md`, skill S0/S1):

  1. **Data** — the string is the initial algebra `SnocList ℤ ℤ` of `F X = ℤ + X × ℤ`
     (`Fredy.A6_SnocList`, characters encoded as `Int`); `wrap x` is a single-character string,
