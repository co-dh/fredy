/-
  ...
  (`Fredy/ColimitPositive.lean`).  Together with the GENERIC constructor
  ...
  (`laxColimHasBinaryCoproducts`, `Fredy/LaxColimitCoproduct.lean`) to a `DisjointBinaryCoproduct`,
  ...
  iso followed by the SAME-STAGE germ injection `stageInclL (hcop k).inl`.  With (★):
    ...
    * DISJOINT — the intersection's domain is the lax-colimit pullback `pbC` of `(coprInl, coprInr)`.
      ...
      (`Fredy/LaxGermPullbacks.lean`) gives a mediator `m₁ : pbC.pt ⟶ objIncl k pdqₖ.pt` into the
