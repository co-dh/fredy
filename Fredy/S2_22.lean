...
theorem self_le_comp_recip_comp [UnionAllegory 𝒜] {a b : 𝒜} (R : a ⟶ b) :
    R ⊑ (R ≫ R°) ≫ R := by
...
theorem symmetric_transitive_idempotent [UnionAllegory 𝒜] {a : 𝒜} {E : a ⟶ a}
...
theorem capE_symm_idem [UnionAllegory 𝒜] {a p c0 : 𝒜} {F0 : c0 ⟶ a} {G0 : c0 ⟶ p}
...
theorem srcTab_leg_map [UnionAllegory 𝒜] {a c0 c : 𝒜} {F0 : c0 ⟶ a} {f : c0 ⟶ c}
...
theorem srcTab_monic [UnionAllegory 𝒜] {a p c0 c : 𝒜} {F0 : c0 ⟶ a} {G0 : c0 ⟶ p}
...
theorem srcTab_Ueq [UnionAllegory 𝒜] {a p c0 c : 𝒜} {F0 : c0 ⟶ a} {G0 : c0 ⟶ p}
...
/-- **The splitting hypothesis** (§2.169/§2.226), stated over the *same*
    ...
    idempotent of `𝒜` splits as a map — exactly `EffectiveAllegory`'s field, but
    routed through the union allegory's reciprocation/composition. -/
def SplitsSymmIdem (𝒜 : Type u) [UnionAllegory 𝒜] : Prop :=
...
theorem srcTabulation_of_semiSimple_split [UnionAllegory 𝒜]
  ...
  obtain ⟨c, f, hf, hffE, hf1⟩ := hsplit (F0 ≫ F0° ∩ G0 ≫ G0°) hEsym hEidem
