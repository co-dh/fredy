/-
  ...
  Same recipe as `Fredy/L20.lean`/`Fredy/L217.lean` (`Fredy/leetcode.md`, skill S0), for a
  DECISION problem (answer = `Bool`, correctness = a plain `Iff`, no extremum):

  1. **Data** — the array is the initial algebra `SnocList ℕ ℕ` (`Fredy.A6_SnocList`); `wrap x` is
     a single-element array, `snoc xs p` appends `p`.

  2. **Program** — the left-to-right GREEDY scan of B&dM §7.2 / `Fredy.A7_2`: fold state
