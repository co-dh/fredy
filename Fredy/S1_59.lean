...
def IsNormalSubobject [HasZeroObject 𝒞] [HasEqualizers 𝒞] {A B : 𝒞}
  ...
  ∃ (C : 𝒞) (f : B ⟶ C), Nonempty (IsIso (kernelMap f)) ∧ kernelMap f = m

/-- An ABELIAN CATEGORY: regular, additive, every subobject is normal. -/
class AbelianCategory (𝒞 : Type u) [Cat.{v} 𝒞]
    extends RegularCategory 𝒞, HalfAdditiveCategory 𝒞, HasZeroObject 𝒞 where
  all_normal : ∀ {A B : 𝒞} (m : A ⟶ B), Mono m → IsNormalSubobject m hm
