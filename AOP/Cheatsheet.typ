...
#laws(
  [$(-)^degree tack.l (-)^degree$], [$R^degree subset S <==> R subset S^degree$ #h(1fr) (self-adjoint order-iso)],
  [$(-) inter R tack.l R => (-)$], [$X inter R subset S <==> X subset (R => S)$],
  [$T(-) tack.l T backslash (-)$], [$T X subset U <==> X subset T backslash U$],
  [$(-)T tack.l (-) slash T$], [$X T subset U <==> X subset U slash T$],
  [$(-) minus S tack.l S union (-)$], [$R minus S subset X <==> R subset S union X$],
  [$f^degree (-) tack.l f(-)$], [shunting: $f^degree R subset S <==> R subset f S$],
  [$(-) f tack.l (-) f^degree$], [shunting: $R f subset S <==> R subset S f^degree$],
  [$"dom" tack.l C |-> C Pi$], [$"dom" R subset C <==> R subset C Pi$ #h(1fr) (dually $"ran" tack.l C |-> Pi C$)],
  [$(-)^* tack.l$ incl.], [$R^* subset X <==> R subset X$ for preorders $X$ #h(1fr) (reflection)],
...
#table(
  ...
  [$(-)^degree$], [$=$], [$=$], [$=$], [involution; $(R S)^degree = S^degree R^degree$],
  [$S(-)$], [mono], [$subset$; $=$ if $S$ simple], [$=$], [$S nothing = nothing$],
  [$(-)S$], [mono], [$subset$; $=$ if $S S^degree subset 1$], [$=$], [],
  [relator $F$], [mono], [$subset$; $=$ on corefl.], [$supset$], [$(F R)^degree = F(R^degree)$; maps $|->$ maps; $F("dom" R) = "dom"(F R)$],
  [$T backslash (-)$], [mono], [$=$], [$supset$], [antitone in $T$: $(R union S) backslash T = (R backslash T) inter (S backslash T)$],
  [$"dom", "ran"$], [mono], [], [$=$], [],
