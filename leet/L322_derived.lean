/-
  ...
  `leet/L322.lean` computes Coin Change with a FUEL recursion whose body folds `omin` over the
  ...
  (`AOP/A6_GenFold.lean`), with carrier `C := List (Option Nat)` — a course-of-values fold
  whose step reads the WHOLE table of previous results.

  The amount axis `0, 1, …, amount` is `SnocList Unit Unit ≅ ℕ` (`SL.snocs`/`SL.natOf`,
  `AOP/A6_8_Tupling.lean`), so tabulating over amounts is a snoc-fold with `L = E = Unit`.  The
...
import leet.L322
import leet.L322_dp   -- reuse the A9_2-derived Coin-Change optimality (solve_correct_inf)
