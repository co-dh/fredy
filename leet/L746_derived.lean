/-
  ...
  `leet/L746.lean` WRITES the pair-carrying DP fold `foldFn` (state `(reachHere, reachPrev)`) by
  hand and then verifies it against the relational spec.  This file instead makes that fold EMERGE
  from `Freyd.Alg.RelSet.SL.tupling` (`AOP/A6_8_Tupling.lean`), exactly as `L70_derived` /
