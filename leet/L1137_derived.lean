/-
  ...
  `leet/L70_derived.lean` linearized a second-order recurrence by carrying a PAIR; Tribonacci is
  third-order (`trib (n+3) = trib (n+2) + trib (n+1) + trib n`, `LC1137.trib`), so we carry a
  TRIPLE.  The tupling law (`AOP/A6_8_Tupling.lean`) is unchanged — its carrier `C₁ × C₂` is
...
/-! ## Running / cross-checking the emergent fold against `leet/L1137.lean`
