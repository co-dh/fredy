/-
  ...
  table precomputed into a HASH MAP for `O(1)`-expected lookup (upgrading `leet/L763.lean`'s two
  `O(n²)` `lastPos` list scans to an `O(n)`-expected two-pass sweep).

  `leet/L763.lean` writes the greedy pass-2 scan `scanFn whole` by hand: a left-to-right fold over
  ...
  character, so pass 2 is `O(n²)`.  Here:
    ...
    * **Pass 2**, the greedy sweep, EMERGES via `SL.snocFold_unique` (`AOP/A6_GenFold.lean`) as the
...
/-! ## Running / cross-checking the emergent hash program against `leet/L763.lean`
