/-
  ...
  `leet/L66.lean` hand-writes `plusOneRev`, the carry-ripple over the LITTLE-ENDIAN digit list:
  ...
  **The paramorphism-as-fold trick**: widen the carrier to `C := List Int × List Int`, pairing
  ...
  `CL.consFold_unique` (`AOP/A6_GenFold.lean`) EMITS `pr` as `cataR (consScalarAlg g st)` — one
