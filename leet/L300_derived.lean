/-
  ...
  `leet/L300.lean` writes an O(n²) tabulating DP by hand (state `(records, best)` where `records`
  ...
  1. **The program EMERGES from the fold law.**  The `tails` fold is a catamorphism with carrier
     ...
     the general-carrier law `Freyd.Alg.RelSet.SL.snocFold_unique` (`AOP/A6_GenFold.lean`) produces
