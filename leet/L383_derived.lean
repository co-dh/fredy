/-
  ...
  `leet/L383.lean`'s `canBuildFn ransom magazine := ransom.all (fun c => decide (countL ransom c ≤
  countL magazine c))` is a `List.all` scan — head-first, `(c :: cs).all p = p c && cs.all p` — the
  exact shape of a `ConsList` cons-fold (`CL.consFold_unique`, `AOP/A6_GenFold.lean`), reshaping
