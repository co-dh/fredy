/-
  ...
  Unlike `leet/L152.lean` (which WRITES the triple-carrying fold `foldFn` by hand and then verifies
  it against the specification `subProd`), this file makes the fold EMERGE via the tupling law
  (`AOP/A6_8_Tupling.lean`, `SL.tupling`).  The Maximum-Product sweep must track THREE running
  ...
  `leet/L152.lean`'s optimality proof (`solve_correct`) — NOT re-proved here — shows the emergent
...
/-! ## The tupling ANSATZ and its FORCED base/step
  ...
  insight.  The ansatz itself is `leet/L152.lean`'s `foldFn` (imported), so we read the base/step
...
/-- The derived program: the emergent triple-fold followed by the projection onto `best` (the third
    component).  This is `leet/L152.lean`'s `solve`, but with the fold PRODUCED by the tupling law
    rather than written by hand. -/
def derivedSolve : LC152.Arr ⟶ LC152.dZ :=
  cataR (pairAlg g step) ≫ graph (fun t : Int × Int × Int => t.2.2)

/-- The derived program equals `leet/L152.lean`'s hand-written `solve`.  `cataR (pairAlg g step)`
...
/-! ## Running / cross-checking the emergent fold against `leet/L152.lean`
